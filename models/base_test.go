...
import (
	"fmt"
	"os"
	"git.betfavorit.cf/vadim.tsurkov/kuberweb/libstring"
	"git.betfavorit.cf/vadim.tsurkov/kuberweb/libunix"
...
func newDbForTest(t *testing.T) *sqlx.DB {
	...
	db, err := sqlx.Connect("postgres", fmt.Sprintf("postgres://%v@%v:%v/kuberweb-test?sslmode=%v", pguser, pghost, pgport, pgsslmode))
	if err != nil {
